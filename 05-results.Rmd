...
6. 
```{r}
#Relate the violation code and the violation description
violation <- distinct(restaurant, VIOLATION.CODE, 
                      .keep_all = TRUE) %>%
  select(VIOLATION.CODE, VIOLATION.DESCRIPTION) 

violation <- violation[order(violation$VIOLATION.CODE),]

#Choose a random sample of violation type and the corrsponding observations
sample_index <- sample(c(1:nrow(violation)), size=10)

violation_sample <- violation[sample_index,]

violation_vector <- as.vector(violation_sample$VIOLATION.CODE)

rest_sample <- restaurant %>%
  filter(VIOLATION.CODE %in% violation_vector)
```
...
type_violation <- type_violation %>%
  mutate(cus_abb=substr(CUISINE.DESCRIPTION,0,3))
```

```{r}
ggplot(type_violation,aes(x=CUISINE.DESCRIPTION,y=prop,
                          fill=VIOLATION.CODE)) +
  geom_bar(stat="identity") 
```
